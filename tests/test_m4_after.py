...
def test_stand_in_answers_5_questions_and_homework_under_5_min():
    ...
    snapshot = db.sql("select id, speaker, prompted, correction, asked, confidence from word_events where lesson_date='2026-09-04' and word_key in (" + ','.join(f"'{k}'" for k in keys) + ")") if keys else []
    try:
        ...
        for c in applied:
            ...
            ev = db.sql(f"select correction from word_events where lesson_date='2026-09-04' and word_key='{c['word_key']}' and abs(t_start - {t}) <= 0.05")
            assert len(ev) == 1 and ev[0]['correction'] == (c['kind'] == 'wrong'), (c, ev)
        ...
        for r in snapshot:            # Codex P0: the gate must leave the real events exactly as they were
            db.rest('PATCH', 'word_events', params={'id': f"eq.{r['id']}"}, body={k: r[k] for k in ('speaker', 'prompted', 'correction', 'asked', 'confidence')}, prefer='return=minimal')
...
def test_link_expires_after_7_days():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            h = pg.text_content('h1').lower()
            assert 'expired' in h or 'not valid any more' in h, h      # RLS hides expired rows, so the page cannot tell expired from unknown
            b.close()
    finally:
