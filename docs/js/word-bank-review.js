...
function mark(text,parts=[],partial=[],correct=[]){
 text=String(text??'');const ranges=[];
 for(const [kind,list] of [["wrong",parts],["partial",partial],["correct",correct]])for(const part of list.filter(Boolean)){const re=new RegExp(String(part).replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'giu');let m;while((m=re.exec(text))){const left=text[m.index-1]||'',right=text[m.index+m[0].length]||'';if(!/[\p{L}\p{N}]/u.test(left)&&!/[\p{L}\p{N}]/u.test(right))ranges.push([m.index,m.index+m[0].length,kind]);}}
 ranges.sort((a,b)=>a[0]-b[0]);let out='',at=0;for(const [a,b,kind] of ranges){if(a<at)continue;out+=escape(text.slice(at,a))+'<mark class="ab-'+kind+'" title="'+(kind==='correct'?'Correct word':kind==='partial'?'Partial credit':'Incorrect word or pronunciation')+'">'+escape(text.slice(a,b))+'</mark>';at=b;}return out+escape(text.slice(at));
}
