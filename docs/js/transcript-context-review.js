...
const used=new Set();for(const e of events){
 ...
 if(e.audio&&!used.has(holder)){used.add(holder);const play=document.createElement('button');play.className='context-listen';play.textContent='▶ Conversation excerpt';play.onclick=()=>{player.pause();player.hidden=false;player.src='../'+e.audio;player.play().catch(()=>{play.textContent='Use the audio player below';});};holder.append(play);}
}
})().catch(()=>{});
