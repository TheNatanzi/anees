...
model=WhisperForConditionalGeneration.from_pretrained(mid,torch_dtype=torch.float16).to("cuda")
...
p=pipeline("automatic-speech-recognition",model=model,tokenizer=proc.tokenizer,feature_extractor=proc.feature_extractor,device=0,torch_dtype=torch.float16,chunk_length_s=8,stride_length_s=(1,1),return_timestamps=True)
...
json.dump({"engine":"oddadmix/whisper-large-v3-turbo-arabic-dialectal","segments":segs},open("data/aug25/whisper_dialectal_8s.json","w",encoding="utf-8"),ensure_ascii=False,indent=1)
open("data/aug25/whisper_dialectal_8s.txt","w",encoding="utf-8").write("\n".join(f"[{int((s['start'] or 0)//60):02d}:{int((s['start'] or 0)%60):02d}] {s['text']}" for s in segs))
