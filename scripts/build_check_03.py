...
gpt = json.load(io.open(R + 'openai_clips_prompt2.json', encoding='utf-8'))
...
for w in wins:
    ...
    pair = [('eleven', ele_text(st, en)), ('chatgpt', gpt_text(i))]
...
.cols{{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px;margin:8px 0}}
...
<p class="lead">Same 20 clips. Two engines wrote down the words, shown as A and B in a random order on every row. No names, no pauses, just the words. Play the clip, then tap the one that matches what was said best. "Same" if they tie, "Both wrong" if neither is close.</p>
...
const K='anees-check-03-v2';let st={{}};try{{st=JSON.parse(localStorage.getItem(K)||'{{}}')}}catch(e){{}}
...
print('check03 rows', n, 'KB', len(page) // 1024, 'eleven first in', sum(1 for v in key.values() if v[0] == 'eleven'), 'rows')
