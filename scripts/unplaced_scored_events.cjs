...
const dates=new Set(process.argv.slice(2)),raw=read('docs/data/word-bank-evidence.json').events,applied=R.apply(raw,process.env.ANEES_REVIEW?JSON.parse(fs.readFileSync(process.env.ANEES_REVIEW,'utf8')):read('docs/data/word-bank-review.json'));
